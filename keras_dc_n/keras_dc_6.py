...
def main():
    ...
    model.add(BatchNormalization(epsilon=1e-06,axis=1,momentum=0.9,beta_initializer='zeros',gamma_initializer='ones'))
    model.add(Dropout(0.3))
    model.add(MaxPool2D())

    model.add(Conv2D(64,kernel_size=(5,5),activation='relu',padding='same'))
    model.add(BatchNormalization(epsilon=1e-6,axis=1,momentum=0.9,beta_initializer='zeros',gamma_initializer='ones'))
    model.add(Dropout(0.3))
    ...
    print("train================================")
    best_model=callbacks.ModelCheckpoint("E:/ML/models/keras_dc_6.h5",monitor='val_loss',verbose=1,save_best_only=True,mode='min')
    tbcallbacks=callbacks.TensorBoard(log_dir='E:/ML/.vscode/dc_logs/keras_dc_6',histogram_freq=1,write_graph=True,write_images=False)
    ...
    with open('E:/ML/models/keras_dc_6.yaml','w') as outfile:
        outfile.write(yaml_string)
    model.save_weights('E:/ML/models/keras_dc_6.h5')


def pred_data():
    
    with open('E:/ML/models/keras_dc_6.yaml') as yamlfile:
        loaded_model_yaml=yamlfile.read()
    model=model_from_yaml(loaded_model_yaml)
    model.load_weights('E:/ML/models/keras_dc_6.h5')
